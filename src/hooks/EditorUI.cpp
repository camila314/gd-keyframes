...
#include "../kfnodes/ConnectorNode.hpp"
...
void EditorUI_hook::onKeyframe(CCObject* sender) {
    auto so = this->getSelectedObjects();

    auto lel = LEL;
    if (so->count() > 0) {
        auto ccar = CCArray::create();
        for (auto j = 0u; j < so->count(); j++) {
            auto i = reinterpret_cast<GameObject*>(so->objectAtIndex(j));
            GameObject* newly = lel->addObjectFromString(i->getSaveString());

            ccar->addObject(newly);
            newly->setGameZOrder(314159);
...
std::vector<KeyframeInfo> EditorUI_hook::subdivide(std::vector<KeyframeInfo> frames) {
    std::vector<KeyframeInfo> new_frames;
    for (auto index = 0u; index < frames.size();index++) {
        KeyframeInfo kinfo = frames[index];

        KeyframeInfo k1 = { kinfo.rotate/2, ccp(kinfo.move.x/2, kinfo.move.y/2) };
        KeyframeInfo k2 = { kinfo.rotate/2, ccp(kinfo.move.x/2, kinfo.move.y/2) };
...
std::string EditorUI_hook::collapseFrames(GameObject* src) {
    std::string toPaste;
    ...
    while (connection) {
        ...
        if (!LEL->getAllObjects()->containsObject(kframe)) {
            connection->destroy();
            break;
        }
    ...
    for (auto i = 0u; i < sController->subdivisions(); i++)
        frames = subdivide(frames);

    float duration = sController->duration();
    //frames.push_back({.move=ccp(0,0),.rotate=0});
    for (auto counter = 0u; counter < frames.size();counter++) {
        auto kinfo = frames[counter];

        int newGroup = Cacao::uniqueGroupToObject(original, LEL);

        auto bezierSpacing = sController->bezierMultiplierAtRange((float)counter, frames.size()) ;//* duration;
        auto nextBezierSpacing = sController->bezierMultiplierAtRange((float)counter + 1.0f, frames.size()) ;//* duration;
        ...
        if (kinfo.move.x != 0 || kinfo.move.y != 0) {
            toPaste = toPaste + createMoveTrigger(
                original->getPosition()+ccp(currPosition, 0), 
                kinfo.move, newGroup,
                (nextPosition - currPosition) / 300.0f
            ) + ";";
        }
        if (kinfo.rotate != 0) {
            toPaste = toPaste + createRotateTrigger(
                original->getPosition()+ccp(currPosition, 30),
                kinfo.rotate, newGroup,
                (nextPosition - currPosition) / 300.0f
            ) + ";";
        }
...
void EditorUI_hook::onClick(CCObject* sender) {
    std::string toPaste;

    for (auto i = 0u; i < this->getSelectedObjects()->count(); i++) {
        GameObject* src = reinterpret_cast<GameObject*>(this->getSelectedObjects()->objectAtIndex(i));
        if (src->getGameZOrder() != 314159 && ConnectorNode::findFromSource(src))
...
void EditorUI_hook::onSpacing(CCObject* sender) {
...
bool __fastcall EditorUI_hook::initHook(EditorUI* self, edx_t, gd::GJGameLevel* lvl) {
    if (!init(self, lvl))
        return false;
    ...
    auto mitem = gd::CCMenuItemSpriteExtra::create(mybutton, self, menu_selector(EditorUI_hook::onKeyframe));
    menu->addChild(mitem);
    ...
    auto mitem2 = gd::CCMenuItemSpriteExtra::create(mybutton2, self, menu_selector(EditorUI_hook::onClick));
    ...
    auto mitem3 = gd::CCMenuItemSpriteExtra::create(mybutton3, self, menu_selector(EditorUI_hook::onSpacing));
